...
var fnc = undefined;
    ...
    var FNCrypto = function() {
        ...
        self.getKeyBundle = function(url) {
            ...
            keyBundle = self._fetch(self._keyName(url));
            ...
            self._store(self._keyName(url), keyBundle);
            return keyBundle;
        }
        ...
        function isRegistered(url) {
            return self._fetch(self._keyName(url)) !=  undefined;
        }

        /** reverse stack of modification history.
         *
         * For 3.0 sync.
         */
        self._setMod = function(key, modType) {
            var mod = JSON.parse(DB['mod']);
            for (i in mod) {
                if (mod[i].key == key) {
                    delete(mod[i]);
                }
            ...
            mod.push({'key': key,
                     ...
                     'mod':modType});
            DB['mod'] = JSON.stringify(mod);
        }

        /** skeleton for 3.0
        */
        self.getChangesSinceTime = function(time, onRecordCallback){
            var changes = new Array(); // not sure if array or object
            if (time == 0 || !DB['mod']) {
                // dump everything to onRecordCallback
                // How to indicate deleted records?
            } else {
                try {
                    mods = JSON.parse(DB['mod']);
                    for(i=mods.length; i>=0; i--){
                        if (mods[i].time >= time) {
                            // add the change to the list of stuff to send.
                            changes.unshift({'mod': mods[i].mod,
                                            'key': mods[i].key,
                                            'rec': JSON.parse(DB[mods[i].time])});
                        }
                    }
                    // format changes to normalized records?
                    // purge old deleted records?
                    // call onRecordCallback?
                }
                catch(e) {
                    // crap.
                }
        ...
        /** skeleton for 3.0
        */
        self.applyIncomingChanges = function(changes, finishedCallback){
            try {
                for each (change in changes){
                    switch (change.mod){
                        case "m":
                            DB[change.key] = change.rec;
                            break;
                        case "d":
                            delete (DB[change.key]);
                            break;
                    }
                }
...
FNSyncTracker.prototype = {
        ...
        /** For Jetpack, make sure you call the (require('observer-service')).add(callback)
         * for observable elements
         */
        observe: function (alpha, beta) {
            console.debug('########### observable', JSON.stringify(alpha), JSON.stringify(beta));
        },
