...
function xhr(opts) {
  ...
  console.info('Attempting call to', opts.method);
  console.info('                  ', opts.url);
  if (opts.data) {
      console.info('                  ', JSON.stringify(opts.data));
  } else {
  ...
  if (opts.headers) {
    console.info('                   ', JSON.stringify(opts.headers))
...
NotificationApi.prototype = {
  ...
  checkRemotePermission: function() {
    ...
    // add crypto elements
    if (url) {
        let kb = fnc.getKeyBundle(this.window.location.hostname);
  ...
  getToken: function(cb) {
    ...
    console.info('calling to ', prefs[SERVER_PREF] + '/token/');
    xhr({method: 'POST',
  ...
  getQueue: function(token, hostname, request) {
    ...
    if (url) {
      request.result.url = url;
      let kb = fnc.getKeyBundle(hostname);
      request.result['encryptionKey'] = kb.encryptionKey;
      request.result['hmac'] = kb.hmac;
      request.dispatch('success');
      console.debug('getQueue', request)
      return;
    }
