...
NotificationApi.prototype = {
  requestRemotePermission: function() {
    let self = this,
        request = new DOMRequest(this.window);
  ...
  getToken: function(cb) {
    ...
    if (self.storage.token) {
      return cb(self.storage.token);
    }

    xhr({method: 'POST',
         url: prefs[SERVER_PREF] + '/token/',
         success: function(xhr) {
           self.storage.token = JSON.parse(xhr.responseText).token;
           console.log('got a token', self.storage.token);
           cb(self.storage.token);
         }
    });
  },
  ...
  getQueue: function(token, hostname, request) {
    var self = this;
    self.storage.getQueue(function(url) {
      if (url) {
        request.result.url = url;
        request.dispatch('success');
        return;
      ...
      xhr({
        ...
        success: function(xhr) {
          ...
          console.log('new queue', url);
          self.storage.setQueue(url);
